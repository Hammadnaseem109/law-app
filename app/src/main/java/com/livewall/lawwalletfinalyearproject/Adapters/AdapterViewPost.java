...
public class AdapterViewPost extends RecyclerView.Adapter<AdapterViewPost.Viewholder> {
    ArrayList<CommentModelClass> data;
    Context context;

    public AdapterViewPost(ArrayList<CommentModelClass> data, Context context) {
        this.data = data;
    ...
    @NonNull
    @Override
    public AdapterViewPost.Viewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.rvlayoutviewpost,parent,false);
        return new Viewholder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull AdapterViewPost.Viewholder holder, int position) {
...
//        String itemPosition = String.valueOf(holder.getAdapterPosition());
        Glide.with(context.getApplicationContext()).load(temp.getProfileimage()).placeholder(R.drawable.progressbarimage).into(holder.profileimage);
        Glide.with(context.getApplicationContext()).load(temp.getPostimage()).placeholder(R.drawable.progressbarimage).into(holder.postimage);
        holder.commenthereimage.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent=new Intent(context, Commenthere.class);
