...
public class ShowUsertolawyerAdapter extends RecyclerView.Adapter<ShowUsertolawyerAdapter.ViewHolder> {
    ArrayList<AddUserDetailToRealtym> datalist;
    Context context;

    public ShowUsertolawyerAdapter(ArrayList<AddUserDetailToRealtym> datalist, Context context) {
    ...
    @NonNull
    @Override
    public ShowUsertolawyerAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.showuserlistlayout,parent,false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ShowUsertolawyerAdapter.ViewHolder holder, int position) {
     final AddUserDetailToRealtym temp=datalist.get(position);
        holder.name.setText(temp.getName());
        holder.city.setText(temp.getCnic());
        Glide.with(context.getApplicationContext()).load(temp.getImageurl()).placeholder(R.drawable.progressbarimage).into(holder.profileimage);
