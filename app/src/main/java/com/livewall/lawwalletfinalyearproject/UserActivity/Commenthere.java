...
public class Commenthere extends AppCompatActivity {
   ...
   String username,curentuidID;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_commenthere);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            Window window = getWindow();
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            window.setStatusBarColor(ContextCompat.getColor(getApplicationContext(), R.color.borron));
        }
        Clicklistener();
        Intent intent = getIntent();
        postkey = intent.getStringExtra("postkey");
//        Toast.makeText(this, "pos"+postposition, Toast.LENGTH_SHORT).show();
        getComment();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            Window window = getWindow();
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
        ...
        myRef.child(randomkey).setValue(modelClass).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()){
                    Toast.makeText(Commenthere.this, "Sucesfully Updated", Toast.LENGTH_SHORT).show();
                }
            }
        }).addOnFailureListener(new OnFailureListener() {
            @Override
            public void onFailure(@NonNull Exception e) {
                Toast.makeText(Commenthere.this, ""+e.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
...
//                                    GetCommentandPostModelClass  obj=dataSnapshot.getValue(GetCommentandPostModelClass.class);
                            ...
                            Toast.makeText(Commenthere.this, "No Comments", Toast.LENGTH_SHORT).show();
                        }
                    }
                    @Override
